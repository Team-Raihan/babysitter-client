...
const BookedUserForBabySitter = () => {
  const [user] = useAuthState(auth);
  const getData = async () => {
    return await axios.get(`http://localhost:5000/api/order/babysitter/${user?.email}`);
  };
  const {
    data: myBooking,
    isLoading,
    refetch,
    error,
  } = useQuery({ queryKey: ["manageBabySitterBookings", 1], queryFn: getData });
  if (isLoading) {
    return (
      <div className=" mt-10">
        <LoadingData />;
...
export default BookedUserForBabySitter;
